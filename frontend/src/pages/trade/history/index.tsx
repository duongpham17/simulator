        ...
        <Pagination data={[...orders].reverse().filter(el => el.open !== true)}>
            {(el) => 
                <Element key={el._id} onClick={() => onOpenItems(el._id)} selected={openItems.includes(el._id)} style={{"padding": "0.5rem"}}>
                    <Text1 name={`${el.side.toUpperCase()} ' ${ strategy.market_id.toLowerCase()} ' ${el.closed}`} nameColor="default" value={el.profit_loss.toFixed(2)} valueColor={el.profit_loss >= 0 ? "green" : "red"}/>
